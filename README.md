...
- Dashboard foundation is implemented on `dashboard-rewrite`: persisted dashboards, query widgets, chart/table widgets, dashboard settings, JSON export, dashboard-level refresh, coordinate-grid edit mode, collision-safe move/resize, model-level layout validation, Markdig-backed markdown viewer widgets, and a scoped dashboard page controller/state model.
