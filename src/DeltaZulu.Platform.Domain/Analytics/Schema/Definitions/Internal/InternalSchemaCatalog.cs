        ...
        SqlBody: """
            SELECT
                ...
                CASE
                    ...
                    ELSE 'Healthy'
                END AS HealthStatus,
                CASE WHEN ReadCount > 0
                    THEN CAST(DiscardedCount AS DOUBLE) / ReadCount
                    ELSE 0
                END AS DiscardRatio
            FROM (
                SELECT *,
    ...
    public static readonly InternalViewDef SourceHealthSummary = new(
        ...
        SqlBody: """
            SELECT
                ...
                CASE WHEN COALESCE(SUM(ReadCount), 0) > 0
                    THEN CAST(COALESCE(SUM(DiscardedCount), 0) AS DOUBLE) / COALESCE(SUM(ReadCount), 0)
                    ELSE 0
                END AS OverallDiscardRatio
        ...
        Description: "Tenant-scoped aggregate health summary across latest source observations.");
