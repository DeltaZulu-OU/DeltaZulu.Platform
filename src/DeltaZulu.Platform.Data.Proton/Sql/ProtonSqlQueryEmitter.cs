...
public sealed class ProtonSqlQueryEmitter : IRelationalQueryEmitter
{
    ...
    private sealed class ProtonEmitter
    {
        ...
        {
            var (source, columns) = EmitNode(node);
            if (columns is null) return source;
            var stage = NextStage();
        ...
        private string EmitProjection(ProjectionExpr proj)
        {
            ...
            if (proj.Expression is ColumnRef col && col.Name == proj.Alias) return expr;
        ...
        {
            if (lit.Value is null || lit.Kind == LiteralKind.Null) return "NULL";
        ...
        private static string EmitInterval(object value)
        {
            ...
            if (ts.Contains(':') && TimeSpan.TryParse(ts, out var parsed))
            {
                ...
                if (parsed.Days != 0)         parts.Add($"INTERVAL {parsed.Days} DAY");
                if (parsed.Hours != 0)        parts.Add($"INTERVAL {parsed.Hours} HOUR");
                if (parsed.Minutes != 0)      parts.Add($"INTERVAL {parsed.Minutes} MINUTE");
                if (parsed.Seconds != 0)      parts.Add($"INTERVAL {parsed.Seconds} SECOND");
                if (parsed.Milliseconds != 0) parts.Add($"INTERVAL {parsed.Milliseconds} MILLISECOND");
