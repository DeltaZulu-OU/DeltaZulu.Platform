...
The target type-bearing exchange is registry-governed Avro from agent to server, decoded once on the server into Arrow record batches. DuckDB should ingest from Arrow, while the Proton leg should consume the decoded typed stream through Proton's native protocol unless the targeted Proton OSS version verifies Avro/schema-registry ingest without Enterprise gating.

The schema registry is producer-agnostic and defines logical field types, nullability, timestamp precision, duration units, nested-shape policy, and per-backend physical mappings. It must project Avro schemas, Arrow schemas, DuckDB DDL, Proton DDL, KQL metadata, and translator type policy from one authority. See `docs/adr/0014-type-fidelity-registry-and-avro-arrow-ingestion.md`.
...
There is deliberately no degraded-mode fallback from Avro to NDJSON on the main agent-to-server wire. Agents should cache schemas locally, spool Avro during registry/server outages, and fail visibly on schema rejection so consumers do not have to support two primary wire formats indefinitely.
