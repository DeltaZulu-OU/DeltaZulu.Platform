...
namespace DeltaZulu.Platform.Application.Governance.ContentLibrary;

/// <summary>
/// Converts existing Analytics saved-query exports into Governance content-library records.
/// The importer intentionally produces draft-only records: accepted content is still created
/// later through Governance change review and merge gates.
/// </summary>
public sealed class SavedQueryImporter
{
    /// <summary>Converts a batch of existing saved queries into draft-only content-library records.</summary>
    public IReadOnlyList<ImportedContentLibraryRecord> Convert(IEnumerable<SavedQueryExportRecord> savedQueries)
    {
        ...
        ISet<string> usedPaths)
    {
        ...
        {
            warnings.Add("Saved query content is empty; Platform preserves it as draft data and the query-syntax check will fail until edited.");
        }
        ...
        var draftFiles = new[]
        {
            new ImportedDraftFile(
                ...
                ContentType: DraftContentType.AnalyticsQuery,
                Content: queryText),
        };
...
/// <summary>One saved query exported from the existing Analytics product.</summary>
public sealed record SavedQueryExportRecord(
    string SourceId,
    string DisplayName,
    string QueryText,
...
/// <summary>A Governance content-library import result plus draft files that can seed a change.</summary>
...
/// <summary>Draft file payload produced by an import before a Governance change owns it.</summary>
