...
/// Subscribes to the configured <c>alert_dispatch</c> Proton stream and appends each
/// received payload to the lake through <see cref="IAlertLakeWriter"/>.
...
public sealed class AlertMediationService : BackgroundService
{
    ...
    private readonly IAlertLakeWriter _lakeWriter;
    ...
    private async Task HandlePayloadAsync(string payload, CancellationToken ct)
    {
        ...
        for (var attempt = 1; attempt <= maxRetries; attempt++)
        {
            try
            {
                await _lakeWriter.AppendAsync(alert, ct);
                return;
            }
