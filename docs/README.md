...
Users work with detections, issues, cases, pull requests, checks, reviews, and versions. They do not interact with Git branches, staging areas, rebases, merge conflicts, workflow-engine internals, or vendor-specific SIEM terminology. Accepted detection content is stored as canonical files in a Git repository. Drafts, issues, cases, PR/change state, comments, reviews, workflow instances, validation runs, and UI projections are stored in the database.
...
1. **Domain-first UI.** The UI presents detection engineering concepts: detections, cases, issues, PRs, checks, reviews, and versions. It must not become a repository browser or workflow-designer UI.
...
3. **Database owns work in progress.** Issues, cases, PR/change drafts, comments, reviews, workflow state, validation runs, locks, and read models live in the database.
...
8. **Tierless SOC workflow.** Case management and detection-as-code are primary use cases and share the same issue/PR/check/review model.
...
```mermaid
flowchart TD
    A[Create issue or case in database] --> B[Create PR/change in database]
    ...
    I --> J[Link version to issue, case, and PR]
```
...
| [ROADMAP.md](ROADMAP.md) | POC acceptance criteria, implementation phases, and out-of-scope boundaries. |
...
| [ADR-0007](adr/0007-case-as-issue-type.md) | Model cases as a rich issue type. |
...
| Issue | Database-owned work item used for new detection, tuning, bug, test gap, research, documentation, maintenance, or case work. |
| Case | Rich issue type for investigation-driven detection work. |
