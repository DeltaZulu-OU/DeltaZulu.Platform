...
2. Issue is a required step before opening a Proposal, adding indirection.
3. Workflow profile selection asks users to choose governance levels they don't understand.
4. Checks and Reviews are standalone pages despite being state within a Proposal.
...
| Issues + Changes | **Proposals** | Combined. Reason and investigation link become fields on the Proposal. Issues become optional, not a workflow step. |
| Checks | *Inline in Proposal* | No standalone page. Shown inside the Proposal workspace. |
| Reviews | *Inline in Proposal + Home queue* | No standalone page. Pending reviews surface on Home. |
| Versions | **History** | Renamed. Compare and restore. |
| Settings | Settings | Kept as operator-only. |

### Navigation reduces to five items

Home, Detections, Proposals, History, Settings.

### Issue becomes optional

- The "reason for proposal" is a text field on the Proposal.
- The "related investigation" is a URL field on the Proposal.
...
- Users do not choose a workflow profile when opening a Proposal.
...
- Creating a Proposal for a new detection implicitly creates the detection identity.
...
- Standalone Checks and Reviews pages are removed from navigation; their content lives inside the Proposal workspace and Home queue.
...
- ADR-0014 (delegate case management) is simplified further: external case reference becomes a single URL field on the Proposal.
