...
Accepted. Supersedes [ADR 0003](0003-schema-medallion-and-provenance.md) where the two conflict. Extended by [ADR 0014](0014-deltazulu-forward-type-fidelity-registry.md) for wire-format and logical-type fidelity.
...
- Proton support exists for detection SQL/DDL, but Golden-compatible Proton streams should be generated from the same logical schema definitions as lake tables. Current NDJSON publishers are transitional and do not satisfy the target type-fidelity contract.
...
- Drift checks should cover DuckDB/DuckLake, Proton, DeltaZulu.Forward envelope, Arrow, and KQL metadata generation from the shared schema catalog.
