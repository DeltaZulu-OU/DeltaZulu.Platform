...
| H2 | Define detection content model | Introduce versioned detection content separate from saved queries. Include KQL, metadata, severity, confidence, risk score, MITRE labels, entity mapping hints, schedule, suppression policy, and tests. | A detection rule has stable identity, version identity, rule hash, enabled status, and test metadata. Saved queries are not treated as detections unless promoted. | ✅ `DetectionRecord` with versioning, MITRE labels, entity hints, schedule, suppression, and test metadata. `IDetectionRepository` and `DapperDetectionRepository` with SQLite persistence. |
| H3 | Add detection execution records | Create `DetectionRuns` or equivalent. Store rule version, execution window, status, result count, duration, error message, and query hash. | Every detection execution is auditable and replayable. A failed run does not create partial ambiguous alerts. | ✅ `DetectionRunRecord` with all specified fields. `IDetectionRunRepository` and `DapperDetectionRunRepository` with SQLite persistence and tests. |
| H4 | Add `Alerts` table | Persist atomic detection matches as alerts. Store detection/version/run IDs, alert time, source view, source event ID/hash, severity, confidence, risk score, selected evidence JSON, and status. | A matching detection result creates one or more deterministic alert records. Duplicate handling is explicit. | ✅ `AlertRecord` with all specified fields. `IAlertRepository` and `DapperAlertRepository` with batch save, status update, duplicate handling, and tests. |
| H5 | Add entity extraction | Create `AlertEntities`. Normalize user, host, IP, process, file hash, domain, URL, cloud resource, registry key, session ID, and similar entities. | Alerts have canonical entities with type, value, role, specificity weight, criticality weight, and high-fanout flag. | ✅ PERSISTENCE BASELINE: `AlertEntityRecord` with type, value, role, specificity weight, criticality weight, and high-fanout flag. `IAlertEntityRepository` and `DapperAlertEntityRepository` with batch save, entity-value lookup, and tests. Extractor service deferred. |
...
| H7 | Add deterministic candidate generation | Implement SQL-first entity/window correlation: group alerts by entity, time window, risk, source diversity, technique/tactic breadth, and sequence compatibility. | Candidate generation creates explainable `IncidentCandidates` from alerts without Workbench involvement. | ✅ PERSISTENCE BASELINE: `IncidentCandidateRecord` with entity, window, alert count, source diversity, tactic/technique breadth, aggregate risk score, scoring factors, correlation rationale, and status. `CandidateAlertLink` for alert-to-candidate membership. `IIncidentCandidateRepository` and `DapperIncidentCandidateRepository` with entity/status queries, alert links, and tests. Candidate generation SQL builder deferred. |
| H8 | Add candidate evidence builder | Query related logs from golden views around candidate entities and windows. Generate timelines and compact evidence summaries. | A candidate can show contributing alerts, related logs, entities, scoring factors, and correlation rationale. | ✅ PERSISTENCE BASELINE: `CandidateEvidenceRecord` with evidence type, content JSON, and collection timestamp. `ICandidateEvidenceRepository` and `DapperCandidateEvidenceRepository` with batch save and tests. Evidence collection service deferred. |
