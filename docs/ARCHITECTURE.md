...
- `DeltaZulu.Agent` is the collector and `DeltaZulu.Forward` is the protocol it speaks. Like fluentd, one binary is both sender and receiver: edge instances collect and forward; a server-side instance receives and fans out to configured sinks. It forwards raw events into the Bronze `RawEventEnvelope` contract and validates against the registry, but neither the Agent nor Forward is a registry projection target — see [ADR 0014](adr/0014-http-ingestion-type-fidelity-registry.md) for why. Its control plane is [ADR 0012](adr/0012-agent-control-plane-pull-protocol-and-auth.md); its ETW collection boundary is [ADR 0010](adr/0010-etw-collection-and-replay-boundaries.md).
- The lake write path is moving to the Agent: a Quack output sink (HTTP, same shape as its ClickHouse sink) will write DuckLake directly. The platform uses DuckDB.NET until then, and keeps DuckDB.NET for query execution regardless.
- DeltaZulu.Agent owns Quack and Proton output sinks and therefore owns telemetry buffering, batching, retry/backpressure, routing, destination authentication, replay, and delivery state. Quack uploads to DuckDB/DuckLake and Proton publishing use their respective HTTP interfaces. Platform `Data.DuckDb` retains query/schema infrastructure; `Data.Proton` retains detection DDL, deployment, and subscription infrastructure.
...
- Producers today are development seeders; the target production producer is the `DeltaZulu.Agent` collector, alongside broker adapters. Direct XML, CSV, and producer-JSON paths must pass through the same registry-backed normalization checkpoint instead of bypassing type classification. Parser/normalization behavior comes from the external DeltaZulu.Parse NuGet dependency, not an in-repo placeholder.
...
Current platform code scaffolds Proton schemas, detection materialized views, development publishers, and alert subscriptions. Those publishers are compatibility/test infrastructure, not the production telemetry route. Production collection delivery moves to DeltaZulu.Agent output sinks for Quack and Proton. The lake query path currently uses DuckDB.NET. Live sink validation, durable agent delivery/replay, generated target schemas, and registry-backed physical-schema drift checks are still required before this path is complete.
