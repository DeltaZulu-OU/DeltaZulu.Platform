...
- `DeltaZulu.Forward` is the collector: the agent-side producer that forwards raw events into the Bronze `RawEventEnvelope` contract. It validates its output against the registry but is not a registry projection target, and its envelope encoding is internal to the collector. Its control plane is [ADR 0012](adr/0012-agent-control-plane-pull-protocol-and-auth.md); its ETW collection boundary is [ADR 0010](adr/0010-etw-collection-and-replay-boundaries.md).
...
- Producers today are development seeders; the target production producer is the `DeltaZulu.Forward` collector, alongside broker adapters. Direct XML, CSV, and producer-JSON paths must pass through the same registry-backed normalization checkpoint instead of bypassing type classification. Parser/normalization behavior comes from the external DeltaZulu.Parse NuGet dependency, not an in-repo placeholder.
