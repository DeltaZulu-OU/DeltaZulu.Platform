...
| 3C | Establish type-fidelity registry and transport | Producer-agnostic schema registry, DeltaZulu.Forward agent wire, Arrow server batches, generated DuckDB/Proton DDL, generated KQL metadata, and governed JSON/NDJSON edge projections. | US-03, US-12, US-26 |
...
| 3C | **In progress** | ADR 0014 is proposed. The first registry contract is scaffolded with producer-agnostic logical field families, nullability, timestamp/duration precision, integer width, nested/dynamic shape metadata, and DeltaZulu.Forward/Arrow/DuckDB/Proton/KQL projection mappings. Current NDJSON codecs and Proton HTTP JSON publishers remain transitional. Remaining work: DeltaZulu.Forward envelope schemas and agent spooling, Arrow server batches, generated DuckDB/Proton physical schemas, KQL metadata/type policy generation, Proton OSS capability verification, and governed JSON edge projections. |
...
| 3C | Establish the type-fidelity registry and replace NDJSON as the type-bearing transport. The registry defines logical field types and projects DeltaZulu.Forward envelope schemas, Arrow in-memory schemas, DuckDB DDL, Proton DDL, KQL metadata, and translator policies. | ADR 0014 accepted or explicitly kept proposed with blocking verification tracked. | Registry contains timestamp, duration, integer, decimal, UUID, IP, nested/dynamic, nullability, and backend mapping rules; DeltaZulu.Forward agent wire and spooling are designed; server decodes DeltaZulu.Forward MessagePack envelopes to Arrow; DuckDB append can ingest Arrow; Proton typed ingest path is verified or the native-protocol hedge is mandatory; JSON/NDJSON remains only as governed ingress/egress/DLQ/debug edge. |
...
1. Define the producer-agnostic logical type taxonomy and schema registry contract for fields, nullability, precision, duration units, nested shapes, and backend mappings. **Status: in progress; initial domain contract and projection mapping tests are in place.**
2. Generate DeltaZulu.Forward envelope schemas for the agent-to-server wire and define schema-version pinning for buffered replay.
3. Design agent DeltaZulu.Forward spooling and replay behavior, including ordering, local buffer format, registry outage handling, and visible schema-rejection failures.
4. Decode DeltaZulu.Forward MessagePack envelopes once on the server into Arrow record batches and define the fan-out boundary for DuckDB and Proton.
...
7. Verify targeted Proton OSS support for schema-registry ingest; if absent or Enterprise-gated, make the native-protocol typed ingest leg mandatory.
