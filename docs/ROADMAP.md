...
|---|---|---|
| Analytics | Feature-rich, not yet Operations-ready | KQL translation at 70.6% coverage (226/320 constructs), schema browser, query history, saved queries, visualizations (ECharts), dashboards (full CRUD with chart/table/markdown widgets, layout, refresh, import/export), Monaco editor with schema-aware metadata. Shared analytics execution contract now serves interactive, dashboard, and governance validation dry-run callers. Still needs curated analytics, threat-hunting/evidence workflow surfaces, and alert/candidate pivots. |
