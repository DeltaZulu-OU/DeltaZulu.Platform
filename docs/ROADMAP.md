...
| 3C | Establish type-fidelity registry and transport | Producer-agnostic schema registry, DuckDB.NET lake access, the Agent's planned Quack HTTP output sink, Proton HTTP streaming, generated DuckDB/Proton DDL, and generated KQL metadata. | US-03, US-12, US-26 |
...
4. Implement the [Quack](https://duckdb.org/docs/current/quack/overview) HTTP output sink on `DeltaZulu.Agent` so the server-side Agent writes DuckLake directly. This moves the ingestion write path out of the platform; `Data.DuckDb` keeps DuckDB.NET for query execution.
5. Validate log upload, batching, authentication, timeout, retry, replay, backpressure, and idempotency behavior for the Agent's Quack sink.
