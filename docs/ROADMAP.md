...
| 3C | Establish type-fidelity registry and transport | Producer-agnostic schema registry, temporary DuckDB.NET lake access, planned Quack HTTP migration, Proton HTTP streaming, generated DuckDB/Proton DDL, and generated KQL metadata. | US-03, US-12, US-26 |
...
| 3C | **In progress** | ADR 0014 is accepted. The registry contract has producer-agnostic logical field families, nullability, timestamp/duration precision, integer width, nested/dynamic shape metadata, and DuckDB/Proton/KQL mappings. DuckDB.NET is the temporary lake adapter and Proton HTTP publishing already exists. Remaining work: Quack HTTP lake integration, generated DuckDB/Proton physical schemas, KQL metadata/type policy generation, payload validation, retry/backpressure/replay policies, and live HTTP integration coverage. |
...
3. Keep current lake behavior working through DuckDB.NET while preventing Application and Web code from depending on embedded-connection details.
4. Implement the future DuckDB/DuckLake HTTP adapter against [Quack](https://duckdb.org/docs/current/quack/overview) behind the existing `Data.DuckDb` boundary.
5. Validate log upload, batching, authentication, timeout, retry, replay, backpressure, and idempotency behavior against Quack.
6. Validate Proton HTTP publishing and streaming with the same operational concerns, including durable cursoring and dead-letter recovery.
...
9. Add opt-in live integration tests for both HTTP destinations and document migration/rollback from DuckDB.NET to Quack.
