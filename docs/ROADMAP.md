...
- Data model is settled: DuckDB is the append-only data lake (Bronze/Silver/Gold events, Alerts, AlertEntities); a separate operations SQLite stores mutable incident-candidate lifecycle state. Current code has alerts scaffolded in the SQLite app-state database — migration to the DuckDB alert lake plus deterministic materialization is required before claiming durable alert delivery.
...
- The shared analytics execution contract (Phase 2) is complete. Curated analytics persistence and promotion (Phase 3) are complete. NRT detection foundation (Phase 3A) is complete, and Phase 3A+ is currently a **runtime scaffold**. The next meaningful progress is hardening that integration path into the target streaming ETL by adding durable cursoring/DLQ/replay, validating it against a live Proton instance, then completing **Phase 3B** (alert storage migration to the DuckDB alert lake) and **Phase 4** (executable detection projection).
...
| 7 | **Scaffolded** | `AlertMediationService` consumes Proton alert-dispatch payloads and writes through `IAlertSink`; missing NRT MV polling/threshold evaluation, cursor commits, durable retry/dead-letter/replay behavior, deterministic deduplication, and complete DuckDB alert-lake schema migration. |
