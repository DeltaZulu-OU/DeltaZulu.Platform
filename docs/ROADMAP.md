...
Phases 1, 1A, 2, 3, 3A, and 3B are **complete**. The current roadmap position makes **Phase 3C and Phase 4 the immediate next phases**. Phase 4 (executable detection projection) is underway and depends on Phase 3 promotion metadata, which is already complete. In parallel, the Proton runtime scaffold (Phase 3A+) is hardened toward Phases 6 and 7 via the `improve-proton-support` branch plan below.

Completed phases (1, 1A, 2, 3, 3A, 3B) are omitted from this table. See the phase status table above for their exit criteria and completion notes.
