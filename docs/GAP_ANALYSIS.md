...
| P1 | Merge reconciliation needs repair automation after intent detection. | Merge now records a database intent before the accepted-content commit, marks the intent with the resulting commit SHA, and completes it with the version projection; the remaining risk is automatic repair of committed-but-unprojected intents. | Add an operator repair/reconciliation workflow that converts committed unresolved intents into version projections or flags them as safely abandoned. |
...
| Version history | Partial | Compare and restore-as-new-change are available with inline text hunks; add reconciliation repair coverage. |
