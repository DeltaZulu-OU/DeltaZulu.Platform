...
| P1 | Merge reconciliation needs an operator UI surface after repair automation. | Merge now records a database intent before the accepted-content commit, marks the intent with the resulting commit SHA, can repair committed-but-unprojected intents into version projections, and leaves missing-commit/pending intents visible as not automatically repairable. | Add an operator-facing page or command surface for listing unresolved intents and invoking repair safely during operations. |
...
| Version history | Partial | Compare and restore-as-new-change are available with inline text hunks; committed merge-intent repair is covered, but an operator-facing repair surface is still needed. |
| UI | Partial | Nav routes, basic version/check/review/settings views, and a POC user switcher exist; deepen workflow actions and end-to-end UX coverage. |
