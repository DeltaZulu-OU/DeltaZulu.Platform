...
public sealed class SchemaBrowserSampleQueryGuardTests
{
    ...
    [TestMethod]
    public void SchemaBrowser_UsesMudNavMenuForGovernanceSections()
    {
        var source = ReadSchemaBrowserSource();
    ...
    private static string ReadSchemaBrowserSource()
    {
        ...
        {
            var candidate = Path.Combine(directory.FullName, "src", "DeltaZulu.Platform.Web", "Analytics", "Shared", "SchemaBrowser.razor");
        ...
        Assert.Fail("Could not locate src/DeltaZulu.Platform.Web/Analytics/Shared/SchemaBrowser.razor from the test output directory.");
        return string.Empty;
    }
}