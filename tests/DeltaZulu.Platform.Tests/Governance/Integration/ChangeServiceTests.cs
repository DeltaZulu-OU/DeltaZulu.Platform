...
public sealed class ChangeServiceTests : IDisposable
{
    ...
    public async Task UpsertDraftFileAsync_CreatesAndUpdatesFiles_PersistsAcrossScopes()
    {
        ...
        using (var scope = _host.CreateScope())
        {
            ...
            await svc.UpsertDraftFileAsync(changeId, "rule.kql", DraftContentType.AnalyticsQuery, "SigninLogs | take 1", Author, TestContext.CancellationToken);
        }
        ...
        {
            var svc = _host.Resolve<ChangeService>(scope);
            await svc.UpsertDraftFileAsync(changeId, "rule.kql", DraftContentType.AnalyticsQuery, "SigninLogs | where ResultType == 50074", Author, TestContext.CancellationToken);
        }
