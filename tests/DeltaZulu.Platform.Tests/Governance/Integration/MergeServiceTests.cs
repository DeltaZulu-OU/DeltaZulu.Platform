...
public sealed class MergeServiceTests : IDisposable
{
    ...
    public async Task QuickLab_Merge_CommitsToContentStore_CreatesVersion_UpdatesDetection()
    {
        ...
        using (var scope = _host.CreateScope())
        {
            ...
            await changeSvc.UpsertDraftFileAsync(changeId, "rule.kql", DraftContentType.AnalyticsQuery, "SigninLogs | take 1", Author, TestContext.CancellationToken);
        }
    ...
    public async Task ControlledReview_Merge_RequiresChecksAndApproval()
    {
        ...
        using (var scope = _host.CreateScope())
        {
            ...
            await changeSvc.UpsertDraftFileAsync(changeId, "rule.kql", DraftContentType.AnalyticsQuery,
    ...
    public async Task Merge_MarksSiblingChangesAsStale()
    {
        ...
        using (var scope = _host.CreateScope())
        {
            ...
            secondId = second.Id;
            await svc.UpsertDraftFileAsync(secondId, "rule.kql", DraftContentType.AnalyticsQuery, "SigninLogs", Author, TestContext.CancellationToken);
        }
    ...
    public async Task Merge_PartialDraft_PreservesUnchangedAcceptedFiles()
    {
        ...
        using (var scope = _host.CreateScope())
        {
            ...
            await svc.UpsertDraftFileAsync(firstId, "rule.kql", DraftContentType.AnalyticsQuery,
                "SigninLogs | take 1", Author, TestContext.CancellationToken);
        }
