...
public sealed class QuerySyntaxCheckTests
{
    ...
    {
        var check = new QuerySyntaxCheck(new NonEmptyQuerySyntaxValidator());
        var ctx = Ctx(new DraftFileSnapshot("rule.kql", DraftContentType.AnalyticsQuery, "   "));
    ...
    public async Task MixedQueries_ReportOnlyValidatorFailures()
    {
        var check = new QuerySyntaxCheck(new NonEmptyQuerySyntaxValidator());
        var ctx = Ctx(
            new DraftFileSnapshot("rule.kql", DraftContentType.AnalyticsQuery, "SigninLogs"),
            new DraftFileSnapshot("empty.kql", DraftContentType.AnalyticsQuery, ""));
    ...
    public async Task FakeValidatorDiagnostics_AreSurfacedWithoutRuntimeExecution()
    {
        ...
        var ctx = Ctx(new DraftFileSnapshot("rule.kql", DraftContentType.AnalyticsQuery, "bad query"));
