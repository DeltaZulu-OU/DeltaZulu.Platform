...
public sealed class DuckDbQueryEmitter
{
    ...
    public string Emit(RelNode node)
    {
        ...
        bool hasUserLimit = HasLimit(node);
    ...
    private (string Source, string? Columns) EmitScan(ScanNode scan)
    {
        return ($"main.{EscapeIdent(scan.ViewName)}", null);
    }
    ...
    private string EmitBinary(BinaryScalar bin)
    {
        ...
        // We use a helper that wraps the term in replace() calls when it is
        // a string literal. For column references we emit ESCAPE '\\' defensively.
        ...
        return bin.Op switch
        {
            ...
            // Case-insensitive substring — ILIKE with metachar-escaped pattern
            ...
            // has family — delegated to EmitBinaryHas for proper regex escaping
            ...
            // matches regex — case-sensitive per spec §7.14: regexp_matches(lhs, pattern, 'c')
            ScalarBinaryOp.MatchesRegex => $"regexp_matches({left}, {right}, 'c')",
            ScalarBinaryOp.NotMatchesRegex => $"(NOT regexp_matches({left}, {right}, 'c'))",
            ScalarBinaryOp.In => $"({left} IN {right})",
            ScalarBinaryOp.NotIn => $"({left} NOT IN {right})",
    ...
    private static string EmitAgo(List<string> args)
    {
        ...
        return $"(current_timestamp - {args[0]})";
    }
    ...
    /// </summary>
    private string EmitTimespanArg(IReadOnlyList<ScalarExpr> args, int index)
    {
        return EmitScalar(args[index]);
    }
    ...
    private string EmitWindow(WindowScalarExpr win)
    {
        ...
        if (win.Window.PartitionBy.Count > 0)
        {
            ...
            if (win.Window.OrderBy.Count > 0) sb.Append(' ');
        }
    ...
    private static string MapWindowFunction(string kustoName)
    {
        return kustoName.ToLowerInvariant() switch
        {
            "lag" => "lag",
            "lead" => "lead",
    ...
    private string EmitWindowBound(WindowBound bound)
    {
    ...
    private static string EmitTimespan(object value)
    {
        ...
        if (ts.Contains(':'))
        {
            if (TimeSpan.TryParse(ts, out var parsed))
            {
                ...
                if (parsed.Days != 0) parts.Add($"{Math.Abs(parsed.Days)} days");
                if (parsed.Hours != 0) parts.Add($"{Math.Abs(parsed.Hours)} hours");
                if (parsed.Minutes != 0) parts.Add($"{Math.Abs(parsed.Minutes)} minutes");
                if (parsed.Seconds != 0) parts.Add($"{Math.Abs(parsed.Seconds)} seconds");
                if (parsed.Milliseconds != 0) parts.Add($"{Math.Abs(parsed.Milliseconds)} milliseconds");
                if (parts.Count == 0) parts.Add("0 seconds");
        ...
        if (ts.EndsWith("ms")) return $"INTERVAL '{ts[..^2]} milliseconds'";
        if (ts.EndsWith("us")) return $"INTERVAL '{ts[..^2]} microseconds'";
        if (ts.EndsWith("d")) return $"INTERVAL '{ts[..^1]} days'";
        if (ts.EndsWith("h")) return $"INTERVAL '{ts[..^1]} hours'";
        if (ts.EndsWith("m")) return $"INTERVAL '{ts[..^1]} minutes'";
        if (ts.EndsWith("s")) return $"INTERVAL '{ts[..^1]} seconds'";
        return $"INTERVAL '{ts}'";
    }
    ...
    /// </summary>
    private static bool HasLimit(RelNode node)
    {
        return node switch
        {
            LimitNode => true,
